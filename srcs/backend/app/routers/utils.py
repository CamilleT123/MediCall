...
def check_back_angle(landmarks: np.ndarray) -> float:
  """Measure back angle bend """
  ...
  knee_mid =(landmarks[mp.solutions.pose.PoseLandmark.LEFT_KNEE.value] + 
             landmarks[mp.solutions.pose.PoseLandmark.RIGHT_KNEE.value]) / 2  
  
  return calculate_angle(shoulders_mid, hips_mid, knee_mid)
...
def get_all_measurements(landmarks: np.ndarray) -> Dict[str, float]:
  ...
  measurements = {
      ...
      'calculate_distance_between_shoulders': calculate_distance_between_shoulders(landmarks)
  }
  return measurements